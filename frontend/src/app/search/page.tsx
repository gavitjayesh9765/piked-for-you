...
export default async function SearchPage({
  ...
  const [categories, results] = await Promise.all([getCategories(), search(q)]);

  const hasQuery = q.trim().length > 0;
  const isEmpty = hasQuery && results.total === 0 && !results.brands.length && !results.categories.length;
