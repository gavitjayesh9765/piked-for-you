...
export default async function AdminDashboard() {
  // One round trip each, in parallel — three sequential awaits made the
  // slowest endpoint the page's floor.
  const [metrics, products, logs] = await Promise.all([
    safe(getMetrics, EMPTY_METRICS),
    safe(() => listProducts({ page: 1 }), {
      items: [],
      total: 0,
      page: 1,
      pageSize: 25,
    ...
    safe(() => listLogs(undefined, 1), { items: [], total: 0, hasMore: false }),
  ]);
